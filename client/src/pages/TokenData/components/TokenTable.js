          ...
          <table className="w-full border-collapse">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gradient-to-r from-[#DD845A] to-[#D3B04D]">
                {Object.keys(filterColumns(tokens[0] || {})).map((key) => (
                  <th
                    key={key}
                    className={`px-6 py-4 text-sm font-semibold text-white first:rounded-tl-lg last:rounded-tr-lg whitespace-nowrap ${getColumnAlignment(key)}`}
                  >
                    {key
                      .replace(/_/g, " ")
            ...
            <tbody className="divide-y divide-amber-100">
              {tokens.map((token, index) => (
                ...
                >
                  {Object.entries(filterColumns(token)).map(([key, value], cellIndex) => (
                    <td 
                      key={cellIndex} 
                      className={`px-6 py-3 text-sm whitespace-nowrap ${getColumnAlignment(key)} text-gray-700`}
                    >
